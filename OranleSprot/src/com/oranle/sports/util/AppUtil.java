...
public class AppUtil
{
    ...
    public static int getDefaultLoadFactor()
    {
        if (_loadFactor == null)
        {
            Integer integer = Integer.valueOf(0xf & context.getResources()
...
//            TLog.error(e.getMessage());
            Log.e(TAG, "get package info exception, NameNotFoundException: " + e.getMessage());
        }
        return false;
    }
...
//            TLog.error(e.getMessage());
    ...
    {
        Intent mainIntent = context.getPackageManager().getLaunchIntentForPackage(
                packageName);
        if (mainIntent == null)
        {
...
//            TLog.log("Action:" + mainIntent.getAction());
            Log.d(TAG, "open app action : " + mainIntent.getAction());
        }
    ...
    public static void copyTextToBoard(String string)
    {
        if (TextUtils.isEmpty(string))
            return;
        ClipboardManager clip = (ClipboardManager) context.getSystemService(
                Context.CLIPBOARD_SERVICE);
        clip.setText(string);
//        AppContext.showToast(R.string.copy_success);
        ToastUtil.showToast(((Activity)context), "copy success！");
    }
